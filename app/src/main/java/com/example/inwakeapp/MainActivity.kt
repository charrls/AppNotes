package com.example.inwakeapp

import MainNav
import android.os.Bundle
import android.util.Log
import androidx.activity.ComponentActivity
import androidx.activity.compose.setContent
import androidx.activity.enableEdgeToEdge
import androidx.activity.viewModels
import androidx.compose.foundation.layout.fillMaxSize
import androidx.compose.foundation.layout.padding
import androidx.compose.material3.Scaffold
import androidx.compose.material3.Text
import androidx.compose.runtime.Composable
import androidx.compose.ui.Modifier
import androidx.compose.ui.tooling.preview.Preview
import androidx.lifecycle.ViewModelProvider
import androidx.navigation.compose.rememberNavController
import com.example.inwakeapp.data.NotesDatabase
import com.example.inwakeapp.repository.NotesRepository
import com.example.inwakeapp.ui.theme.InWakeAppTheme
import com.example.inwakeapp.viewmodel.NotesViewModel
import com.example.inwakeapp.viewmodel.NotesViewModelFactory
import com.example.inwakeapp.views.HomeScreen



class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContent {
            InWakeAppTheme {
                val navController = rememberNavController()

                val notesDAO = NotesDatabase.getDatabase(applicationContext).notesDAO()
                val notesRepository = NotesRepository(notesDAO)

                val notesViewModel: NotesViewModel = ViewModelProvider(
                    this,
                    NotesViewModelFactory(notesRepository)
                )[NotesViewModel::class.java]

                Log.d("MainActivity", "NotesViewModel initialized: $notesViewModel")

                MainNav(navController, notesViewModel)
            }
        }
    }
}

